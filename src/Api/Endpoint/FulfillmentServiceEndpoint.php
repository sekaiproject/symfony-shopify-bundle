...
class FulfillmentServiceEndpoint extends AbstractEndpoint
{
     ...
     */
    public function findAll(array $query = ['scope' => 'all'])
    {
    ...
    {
        $request = new PostJson('/admin/fulfillment_services.json', ['fulfillment_service' => $fulfillmentService->toArray()]);
        $response = $this->send($request);
    ...
    {
        $request = new PutJson('/admin/fulfillment_services/'.$fulfillmentServiceId.'.json', ['fulfillment_service' => $fulfillmentService->toArray()]);
        $response = $this->send($request);
