...
class ProvinceEndpoint extends AbstractEndpoint
{
     ...
     */
    public function findByCountry($countryId, array $query = [])
    {
     ...
     * @return int
     */
    public function countByCountry($countryId, array $query = [])
    {
        $request = new GetJson('/admin/countries/'.$countryId.'/provinces.json', $query);
        $response = $this->send($request);
    ...
    {
        $request = new PutJson('/admin/countries/'.$countryId.'/provinces/'.$provinceId.'.json', ['province' => $province->toArray()]);
        $response = $this->send($request);
