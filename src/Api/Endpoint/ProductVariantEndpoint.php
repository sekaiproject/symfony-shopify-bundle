...
class ProductVariantEndpoint extends AbstractEndpoint
{
    ...
    {
        $request = new PostJson('/admin/products/'.$productId.'/variants.json', ['variant' => $variant->toArray()]);
        $response = $this->send($request);
    ...
    {
        $request = new PutJson('/admin/variants/'.$variantId.'.json', ['variant' => $variant->toArray()]);
        $response = $this->send($request);
