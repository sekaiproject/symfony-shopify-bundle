...
class CustomerEndpoint extends AbstractEndpoint
{
     ...
     */
    public function findOrdersForCustomer($customerId, array $fields = [])
    {
    ...
    {
        $request = new PostJson('/admin/customers.json', ['customer' => $customer->toArray()]);
        $response = $this->send($request);
    ...
    {
        $request = new PutJson('/admin/customers/'.$customerId.'.json', ['customer' => $customer->toArray()]);
        $response = $this->send($request);
