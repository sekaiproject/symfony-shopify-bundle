...
class OrderEndpoint extends AbstractEndpoint
{
     ...
     */
    public function findOne($orderId, array $fields = [])
    {
    ...
    {
        $request = new PostJson('/admin/orders.json', ['order' => $order->toArray()]);
        $response = $this->send($request);
    ...
    {
        $request = new PutJson('/admin/orders/'.$orderId.'.json', ['order' => $order->toArray()]);
        $response = $this->send($request);
